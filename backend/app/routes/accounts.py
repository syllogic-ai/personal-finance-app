...
def update_account(
    account_id: UUID, updates: AccountUpdate, db: Session = Depends(get_db)
):
    account = db.query(Account).filter(Account.id == account_id).first()
    if not account:
        raise HTTPException(status_code=404, detail="Account not found")
...
def delete_revolut_default_accounts(db: Session = Depends(get_db)):
    """
    Permanently delete all "Revolut default" accounts.
    This is a hard delete - use with caution.
    """
    from app.models import Transaction
    
    # Find all revolut_default accounts
...
def get_all_accounts_debug(db: Session = Depends(get_db)):
    """
    Debug endpoint to see ALL accounts in the database (including inactive ones).
...
def recalculate_account_balance(account_id: UUID, db: Session = Depends(get_db)):
    """
...
def delete_empty_accounts(db: Session = Depends(get_db)):
    """
    Automatically delete all accounts that have 0 transactions.
...
def restore_seed_accounts(db: Session = Depends(get_db)):
    """
